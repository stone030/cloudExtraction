...
def is_night_time(image):
    ...
    # If the average brightness is below a certain threshold,
    # then the image was taken at night time.
    threshold = 100
    if average_brightness < threshold:
        # os.system('pm-suspend && /home/pi/.virtualenvs/cv/bin/python3.7 main\ final_contours.py')
        print('-------------\nnight zone :) good night!\n----------------')
    else:
        return 0
...
CameraCapture = cv2.VideoCapture(0)#"Part2/Live ISS stream record short.mp4") # or import a video like '/dataset/All-Way Pedestrian Crossing.mp4'
...
Near_Oman = 0
#check conditions (open iss.txt and read last line to extract lon/lat coordinates)
Near_Oman = 0
lon = 0
lat = 0
region = ""
...
#             with open("Tile_description.txt", "a") as f:
...
#                 sub += 1
        ...
        frame = FPSC.update(frame, pos=(wf - 200, 50), color=(0, 0, 255), scale=2, thickness=2)[1]
        end = time.time()
#         printnt for 1 image: ')
#         print(end - start)
